...
public class RestaurantCompletedOrdersController {

    private static final String RESTAURANT_ID_ORDERS = "/restaurant/{id}/orders";
    ...
    @GetMapping(RESTAURANT_ID_ORDERS)
    public ModelAndView restaurantPage(
            @PathVariable("id") final Long id
    ) {
        RestaurantDTO restaurantDTO = restaurantMapper.map(restaurantService.findById(id));

        List<Order> orders = orderService.getAllOrdersByRestaurantIdAndStatus(id, OrderStatus.completed);
        ...
        return new ModelAndView("owner_panel/restaurant_completed_orders", modelData);
    }
